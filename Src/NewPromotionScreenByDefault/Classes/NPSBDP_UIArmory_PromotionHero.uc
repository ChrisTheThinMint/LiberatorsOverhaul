...
struct CustomAbilityStatGain
{
	var name AbilityName;
	var name ClassName;
	var array<SoldierClassStatType> StatGains;
};

struct CustomAbilityRequirement
{
	var name AbilityName;
...
var localized string m_strProficiencyUnlock;
var localized string m_strStatGainsHeader;

var localized string m_strStatHP;
var localized string m_strStatShieldHP;
var localized string m_strStatOffense;
var localized string m_strStatDefense;
var localized string m_strStatDodge;
var localized string m_strStatWill;
var localized string m_strStatMobility;
var localized string m_strStatHacking;
var localized string m_strStatSightRadius;
var localized string m_strStatArmorMitigation;
var localized string m_strStatPsiOffense;
var localized string m_strStatCritChance;
...
var config array<name> ClassHasProficiencyRank;
var config array<CustomClassAbilityCost> ClassCustomAbilityCost;

var config array<name> ClassIgnoresAbilityRequirements;
var config array<name> AbilityConsidersTemplateRequirements;

var config array<CustomAbilityRequirement> ClassAbilityRequirements;

var config array<CustomAbilityStatGain> CustomAbilityStatGains;
...
function bool CanPurchaseAbility(int Rank, int Branch, name AbilityName)
{
	...
	if(IsProficiencyBranch(UnitState, Branch))
	{
		return false;
	}
	...
	if(AbilityRanks == 0)
	{				
		return (Rank < UnitState.GetRank() && CanAffordAbility(Rank, Branch) && MeetsAbilityRequirements(UnitState, AbilityName));
	}
	...
	return (Rank < UnitState.GetRank() &&
		...
		(MeetsAbilityRequirements(UnitState, AbilityName)));
}
...
function PreviewAbility(int Rank, int Branch)
{
	...
	{		
		...
		if (AbilityTemplate != none)
		{
			...
			{
				AffixStatGainStrings(Unit, Rank, Branch, AbilityTemplate, AbilityDesc);

				// Is this a proficiency skill?
				if(IsProficiencyBranch(Unit, Branch))
				{
					AffixProficiencyString(Unit, AbilityTemplateManager, AbilityTree, AbilityTemplate, AbilityDesc);
				}
				// Musashi ignore ability prerquisites for heroes
				else if (AbilityTemplate.PrerequisiteAbilities.Length > 0)
				{
					AffixPrerequisiteStrings(Unit, AbilityTemplateManager, AbilityTree, AbilityTemplate, AbilityDesc);
				}
			}
		}
		else
...
simulated function ComfirmAbilityCallback(Name Action)
{
	...
	if(Action == 'eUIAction_Accept')
	{
		...
		if(bSuccess)
		{
			if(HasProficiencyBranch(UpdatedUnit))
			{
				ClassName = UpdatedUnit.GetSoldierClassTemplateName();

				ProficiencyBranch = GetCustomAbilitiesPerRank(ClassName);

				bSuccess = UpdatedUnit.BuySoldierProgressionAbility(UpdateState, PendingRank, ProficiencyBranch);
				
				if(bSuccess)
				{
					`LOG("Liberators - Added proficiency ability after progression ability was purchased");
				}
			}
...
function bool HasProficiencyBranch(XComGameState_Unit UnitState)
{
	local name ClassName;

	ClassName = UnitState.GetSoldierClassTemplateName();

	if(default.ClassHasProficiencyRank.Find(ClassName) > -1)
	{
		return true;
	}
...
function bool IsProficiencyBranch(XComGameState_Unit UnitState, int Branch)
{
	local name ClassName;
	local int AbilitiesPerRank;
	
	ClassName = UnitState.GetSoldierClassTemplateName();
	AbilitiesPerRank = GetCustomAbilitiesPerRank(ClassName);

	if(default.ClassHasProficiencyRank.Find(ClassName) > -1)
	{
		if(Branch == AbilitiesPerRank)
		{
			return true;
		}
	}
...
static function AffixProficiencyString(XComGameState_Unit Unit, X2AbilityTemplateManager AbilityTemplateManager, array<SoldierClassAbilityType> AbilityTree, X2AbilityTemplate AbilityTemplate, out string AbilityDesc)
{
	AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(default.m_strProficiencyUnlock, eUIState_Good) $ "\n" $ AbilityDesc;
}

function AffixStatGainStrings(XComGameState_Unit UnitState, int Rank, int Branch, X2AbilityTemplate AbilityTemplate, out string AbilityDesc)
{
	local name ClassName;
	local string Stats;
	local CustomAbilityStatGain AbilityStats;
	local SoldierClassStatType StatGain;

	ClassName = UnitState.GetSoldierClassTemplateName();

	foreach default.CustomAbilityStatGains(AbilityStats)
	{
		if(AbilityStats.ClassName == ClassName && AbilityStats.AbilityName == AbilityTemplate.DataName)
		{
			foreach AbilityStats.StatGains(StatGain)
			{
				if(Stats != "") 
				{ 
					Stats $= ", "; 
				}
				if(StatGain.StatAmount != 0)
				{
					Stats $= string(StatGain.StatAmount);

					if(StatGain.RandStatAmount > 0)
					{
						Stats $= "-" $ string(StatGain.RandStatAmount);
					}
				}
				switch(StatGain.StatType)
				{
					case eStat_HP : Stats $= " " $ m_strStatHP; break;
					case eStat_Offense : Stats $= " " $ m_strStatOffense; break;
					case eStat_Defense : Stats $= " " $ m_strStatDefense; break;
					case eStat_Dodge : Stats $= " " $ m_strStatDodge; break;
					case eStat_Will : Stats $= " " $ m_strStatWill; break;
					case eStat_Mobility : Stats $= " " $ m_strStatMobility; break;
					case eStat_Hacking : Stats $= " " $ m_strStatHacking; break;
					case eStat_SightRadius : Stats $= " " $ m_strStatSightRadius; break;
					case eStat_ArmorMitigation : Stats $= " " $ m_strStatArmorMitigation; break;
					case eStat_PsiOffense : Stats $= " " $ m_strStatPsiOffense; break;
					case eStat_CritChance : Stats $= " " $ m_strStatCritChance; break;
					case eStat_ShieldHP : Stats $= " " $ m_strStatShieldHP; break;
			...
			if(Stats != "")
			{
				AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(m_strStatGainsHeader $ Stats, eUIState_Good) $ "\n" $ AbilityDesc;
			}
		}
	}
}

static function AffixPrerequisiteStrings(XComGameState_Unit Unit, X2AbilityTemplateManager AbilityTemplateManager, array<SoldierClassAbilityType> AbilityTree, X2AbilityTemplate AbilityTemplate, out string AbilityDesc)
{
	local name ClassName;
	local X2AbilityTemplate PreviousAbilityTemplate;
	local string PrereqAbilityNames;
	local name PrereqAbilityName;
	// Variable for Issue #128
	local string MutuallyExclusiveNames;
	local CustomAbilityRequirement Requirement;

	ClassName = Unit.GetSoldierClassTemplateName();

	if(default.ClassIgnoresAbilityRequirements.Find(ClassName) == -1)
	{
		...
		// Start Issue #128
		if(default.AbilityConsidersTemplateRequirements.Find(AbilityTemplate.DataName) > -1)
		{
		...
		//Look through the new requirement array
		foreach default.ClassAbilityRequirements(Requirement)
		{
			if(Requirement.AbilityName == AbilityTemplate.DataName)
			{
				if(Requirement.Exclusive)
				{
					...
					PreviousAbilityTemplate = AbilityTemplateManager.FindAbilityTemplate(Requirement.RequiredAbility);
		...
		if (MutuallyExclusiveNames != "")
		{
			...
			AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(default.m_strMutuallyExclusive @ MutuallyExclusiveNames, eUIState_Warning) $ "\n" $ AbilityDesc;
		}

		if (PrereqAbilityNames != "")
		{
			PrereqAbilityNames = class'UIUtilities_Text'.static.FormatCommaSeparatedNouns(PrereqAbilityNames);
			AbilityDesc = class'UIUtilities_Text'.static.GetColoredText(default.m_strPrereqAbility @ PrereqAbilityNames, eUIState_Warning) $ "\n" $ AbilityDesc;
		}
	}
	// End Issue #128
}

static function bool MeetsAbilityRequirements(XComGameState_Unit Unit, name NewAbility)
{
	local name ClassName;
	local X2AbilityTemplateManager AbilityTemplateManager;
	local X2AbilityTemplate AbilityTemplate;
	...
	AbilityTemplateManager = class'X2AbilityTemplateManager'.static.GetAbilityTemplateManager();
	AbilityTemplate = AbilityTemplateManager.FindAbilityTemplate(NewAbility);

	//Does the class ignore requirements?
	if(default.ClassIgnoresAbilityRequirements.Find(ClassName) > -1)
	{
		return true;
	}

	//Else, does the soldier meet all ability requirements?
	//We ignore prerequisites from the ability template here, unless we are told to
	if(default.AbilityConsidersTemplateRequirements.Find(NewAbility) > -1)
	{
		if(Unit.MeetsAbilityPrerequisites(NewAbility) == false)
		{
			return false;
		}
	}
	...
	foreach default.ClassAbilityRequirements(Requirement)
	{
		if(Requirement.AbilityName == NewAbility)
		{
			if(Requirement.Exclusive)
			{
				if(Unit.HasSoldierAbility(Requirement.RequiredAbility))
				{
					return false;
				}
			}
